...
import { TEAM_PHOTOS, TeamPhoto } from '../src/data/teamPhotos';
...
function isSamePhoto(p1: { date: string; location: string }, p2: { date: string; location: string }) {
  return normalizeDateStr(p1.date) === normalizeDateStr(p2.date) &&
...
async function main() {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch group page: ${response.status} ${response.statusText}`);
    }
    ...
    console.log(`Parsed ${crawledPhotos.length} valid team photos with metadata.`);
    ...
    for (const photo of crawledPhotos) {
      ...
      const alreadyExists = TEAM_PHOTOS.some(existing => isSamePhoto(existing, photo));
      
      if (alreadyExists) {
        console.log(`Reached already-existing photo: "${photo.date}" at "${photo.location}". Stopping crawl.`);
        stopCrawling = true;
        break;
      }
    ...
    for (const ym of Object.keys(groupedNew)) {
      ...
      if (existingCount === 0 && groupPhotos.length === 1) {
        ...
        for (const existing of existingInMonth) {
          ...
          if (parts.length > 1) {
            const suffixNum = parseInt(parts[parts.length - 1], 10);
    ...
    const finalNewPhotosOrdered = newPhotosToProcess.map(photo => {
      const assigned = newTeamPhotos.find(p => p.date === photo.date && p.location === photo.location);
      if (!assigned) {
        throw new Error(`Failed to match assigned URL for ${photo.date} - ${photo.location}`);
      }
      return assigned;
    });
...
export const TEAM_PHOTOS: TeamPhoto[] = [\n`;
    ...
    console.log(`Successfully wrote ${combinedPhotos.length} photos to ${OUTPUT_PATH}`);
